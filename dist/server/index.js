...
async function registerRoutes(httpServer2, app2) {
  ...
  app2.get("/api/get-ocupacion.php", async (req, res) => {
    try {
      ...
      const campingData = fs.existsSync(campingsFile) ? JSON.parse(fs.readFileSync(campingsFile, "utf-8")) : [];
      const requestedUnit = unidadId ? campingData.find((camping) => camping.id === Number(unidadId))?.name : null;
      const result = requestedUnit ? await pool.query("SELECT fecha_inicio, fecha_fin, unidad FROM reservas WHERE estado != 3 AND unidad = $1", [requestedUnit]) : await pool.query("SELECT fecha_inicio, fecha_fin, unidad FROM reservas WHERE estado != 3");
      res.json(result.rows);
    } catch (error) {
      console.error("Database Error:", error);
      res.status(500).json({ error: error.message });
  ...
  app2.post("/api/crear-reserva.php", async (req, res) => {
    ...
    try {
      ...
      const unitsInType = requestedCamping ? publicCampings.filter((c) => c.typeId === requestedCamping.typeId) : [];
      const isValidAutomaticAssignment = Boolean(autoAsignada) && unitsInType.length > 1;
      if (!requestedCamping || requestedCamping.visible === false && !isValidAutomaticAssignment) {
        return res.status(400).json({
          success: false,
          error: "Esta unidad no est\xE1 disponible para reservas p\xFAblicas."
        });
      }
      const startOnly = fecha_inicio.substring(0, 10);
      const endOnly = fecha_fin.substring(0, 10);
  ...
  app2.put("/api/discount-codes/:id", (req, res) => {
    try {
      ...
      if (index === -1)
        return res.status(404).json({ success: false, error: "C\xF3digo no encontrado" });
  ...
  app2.post("/api/validate-discount", (req, res) => {
    try {
      ...
      if (!found)
        return res.json({ valid: false, mensaje: "C\xF3digo de descuento no v\xE1lido." });
      if (!found.activo)
        return res.json({ valid: false, mensaje: "Este c\xF3digo ya no est\xE1 activo." });
  ...
  app2.post("/api/plan-blocks", (req, res) => {
    try {
      ...
      const blocks = readPlanBlocks();
      const isDuplicate = blocks.some((block) => block.planId === planId && JSON.stringify(block.campingIds.sort()) === JSON.stringify(campingIds.sort()) && block.fechaInicio === normalizedStart && block.fechaFin === normalizedEnd);
      if (isDuplicate) {
  ...
  app2.post("/api/unit-blocks", (req, res) => {
    try {
      ...
      const blocks = readUnitBlocks();
      const isDuplicate = blocks.some((b) => b.unitName === unitName && b.fechaInicio === normalizedStart && b.fechaFin === normalizedEnd);
      if (isDuplicate) {
