...
export async function registerRoutes(
  ...
  app.post("/api/actualizar-reserva.php", async (req, res) => {
    ...
    try {
      ...
      if (cedula !== undefined) { updates.push(`cedula = $${paramIndex++}`); params.push(cedula || null); }
      if (plan) { updates.push(`plan = $${paramIndex++}`); params.push(plan); }
      if (totalNum !== undefined) { updates.push(`total = $${paramIndex++}`); params.push(totalNum); }
      if (abonoNum !== undefined) { updates.push(`abono = $${paramIndex++}`); params.push(abonoNum); }
